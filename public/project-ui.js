export function projectFeatures({api,modal,toast,escape:e,getState,getMe,refreshFiles,refreshPreview,download}){
  ...
  $('archived-sessions').onclick=async()=>{try{const result=await api('/api/session/archives');modal(`<h2>보관된 세션</h2><p>이전 기록을 확인하거나 같은 작업공간의 세션을 다시 이어갈 수 있습니다.</p>${result.archives.map(a=>`<button class="file-row" data-archive="${e(a.id)}"><span>${e(a.title)}</span><small>v${a.revision} · ${a.turns}턴${a.canResume?' · 재개 가능':''}</small></button>`).join('')||'<p>아직 보관된 세션이 없습니다.</p>'}`);document.querySelectorAll('[data-archive]').forEach(b=>b.onclick=async()=>{try{const saved=await api('/api/session/archives/'+b.dataset.archive);modal(`<h2>${e(saved.title)}</h2><p>${e(saved.nativeId||'실행 전')} · v${saved.revision}</p>${saved.turns.map(t=>`<details class="change-detail"><summary>${e(t.authorName)} · ${e(t.prompt)}</summary>${t.items.map(i=>`<pre class="approval-preview">${e(i.text)}</pre>`).join('')}</details>`).join('')}${saved.canResume?'<p>재개하면 현재 세션을 보관하고 이 원본을 이어갑니다. 현재 참여자 모두에게 이 기록이 공유됩니다. 프로젝트 파일은 현재 상태로 유지됩니다.</p><button class="button primary" id="resume-archive">이 세션 이어서 작업</button>':'<p>다른 작업공간의 세션입니다. 호스트에서 해당 폴더를 다시 연 뒤 재개할 수 있습니다.</p>'}`);if($('resume-archive'))$('resume-archive').onclick=async()=>{try{$('resume-archive').disabled=true;await api('/api/session/archives/'+saved.id+'/resume',{});$('modal').close();toast('보관된 원본 세션을 재개했습니다.');}catch(error){$('resume-archive').disabled=false;toast(error.message);}};}catch(error){toast(error.message);}});}catch(error){toast(error.message);}};
  ...
  return {
    ...
    render(state,me){
      ...
      $('file-events').textContent=(state.fileEvents||[]).slice(-20).reverse().map(event=>`${event.actor} · ${{restore:'복구',upload:'업로드','server-start':'서버 시작','server-stop':'서버 중지','instruction-cancelled':'지시 취소','session-resume':'세션 재개'}[event.kind]||event.kind} · ${event.script||event.path}`).join('\n')||'아직 작업 기록이 없습니다.';
