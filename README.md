...
> 현재는 **Codex 전용 프로토타입**입니다. 같은 계정의 독립 실행기 두 개로 A → B → A의 세션 연속성을 확인했습니다. **서로 다른 실제 계정·기기의 세션 재개와 계정별 할당량 분리는 아직 검증하지 않았습니다.**
...
2026-09-11 기준 **자동 테스트 16개 통과**. 세션 원본 보존, 대기열, 초대·실행기 권한, 파일 경로·버전 검사, 중계, Tailscale 설정과 원격 관리자 접근 차단을 검사합니다. 자동 테스트는 실제 AI를 호출하지 않습니다.
...
실제 검증에서는 같은 계정의 독립 실행기 두 개로 대화 속 조건, 이전 도구 결과, 동료가 수정한 조건의 연속성을 확인했습니다. 실제 Tailscale Serve의 HTTPS·실시간 응답도 확인했지만 **동일 PC에서 사설 주소로 접속한 검사**입니다. 다른 물리 기기와 다른 AI 계정의 전체 협업 흐름은 후속 검증 대상입니다. [검증 기록](docs/PROTOTYPE-VALIDATION.md)
