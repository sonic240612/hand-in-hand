...
- **기록 보관:** 세션 원본 내보내기, 보관된 세션 조회, 업로드·복구 내역의 다음 native 턴 적용을 지원합니다.
...
호스트에서 개발 서버를 실행한 뒤 **미리보기 → 개발 서버 연결**에 포트를 입력합니다. 예를 들어 `5173`을 입력하면 호스트의 `127.0.0.1:5173`을 공유합니다. 웹페이지의 상대 경로·스크립트·스타일·WebSocket 갱신을 지원합니다. 개발 서버는 호스트 터미널에서 계속 실행해 두세요. AI 턴의 임시 실행 환경에서 시작한 프로세스는 턴 종료 시 종료될 수 있습니다.
...
**세션 기록**에서 원본 내보내기와 파일 작업 기록을 확인하고, 호스트는 **보관된 세션**을 읽을 수 있습니다. 내보낸 파일에는 공유 대화와 도구 결과가 포함되므로 팀의 공유 자료로 관리하세요. 새 세션을 만들려면 실행 중이거나 대기 중인 작업을 먼저 끝내거나 취소해야 합니다.
...
호스트·연결기를 종료한 뒤 `npm run update`를 실행하면 작업 폴더가 깨끗한지 확인하고 `git pull --ff-only`와 `npm ci`를 실행합니다. 커밋되지 않은 변경이나 실행 중인 기본 호스트가 있으면 중지합니다. `--data`를 사용하는 호스트는 `npm run update -- --data 기존경로`로 검사하세요. 사용자 파일·`.hih`·`.hih-agent`는 삭제하지 않습니다. 업데이트 후 기존 시작 명령으로 재연결합니다. ZIP 연결기는 새 패키지의 코드만 교체하고 `.hih-agent`를 보존하세요.
...
2026-09-12 기준 **자동 테스트 52개 통과**. 세션 원본 보존, 대기열, 초대·실행기 권한, 파일 경로·버전 검사, 중계, Tailscale, 실행 저장소 격리, API 키 차단, 충돌 복구와 중복 재시도, 정상 압축과 중단 기록 복구, 기본 도구의 인증된 전송·승인 대상 구분·하위 세션 원문 보존과 Windows UTF-8 실행 처리를 검사합니다. 추가로 관찰자 권한, 재개 가능한 파일 전송, 파일 복구 충돌, 파일 이벤트의 native 기록 적용, 개발 서버 HTTP·WebSocket 인증과 권한 회수, 별도 Tailscale 미리보기 포트와 이전 연결 정리를 검사합니다. 자동 테스트는 실제 AI를 호출하지 않습니다.
