...
2026-09-11 기준 **자동 테스트 32개 통과**. 세션 원본 보존, 대기열, 초대·실행기 권한, 파일 경로·버전 검사, 중계, Tailscale, 실행 저장소 격리, API 키 차단, 충돌 복구와 중복 재시도, 정상 압축과 중단 기록 복구를 검사합니다. 자동 테스트는 실제 AI를 호출하지 않습니다.
...
- Codex CLI 0.154.0에서 실제 압축 후 별도 실행기로 같은 세션을 재개하고 원문 보존을 확인했습니다. 이미지·Claude 혼합 실행은 현재 검증 범위 밖입니다.
